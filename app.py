...
def login_page():
    policy_number = None  # Initialize policy_number
    if request.method == 'POST':
        ...
        if policy:
            # Set the policy number and re-render the template with it
            policy_number = policy['policy_num']
            return render_template('login.html', policy_number=policy_number)
        else:
            # Render template with an error for invalid login details
...
def index():
    ...
    if policy_number:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM policy_data WHERE policy_num = ?", (policy_number,))
        row = cursor.fetchone()
        conn.close()

        if row:
            column_names = [desc[0] for desc in cursor.description]
            policy_data = dict(zip(column_names, row))
    ...
    response = make_response(render_template('index.html', error=error_message, success=success_message, policy=policy_data, ts=timestamp))

    # Add no-cache headers directly to the response
...
def database():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM policy_data")
    policies = cursor.fetchall()
    conn.close()

    # Process effective and expiry date/time
    processed_policies = []
    for policy in policies:
        ...
        expiry_time = policy.get('expiry_time', '')

        # Combine date and time if both parts are available
        policy['policy_start_datetime'] = f"{effective_date} {effective_time}" if effective_date and effective_time else ''
        policy['policy_end_datetime'] = f"{expiry_date} {expiry_time}" if expiry_date and expiry_time else ''

        processed_policies.append(policy)

    return render_template('database.html', policies=processed_policies)
...
def edit_policy(policy_id):
    ...
    cursor = conn.cursor()
    cursor.execute("""
    SELECT id, first_name, last_name, dob, sex, title, postcode, telephone, email, make, model, 
           vehicle_reg_number, effective_date, effective_time, expiry_date, expiry_time, 
           duration, vehicle_value_from, vehicle_value_to, kgm_insurer_premium, 
           insurance_premium_tax, tempcover_fee, total_charged, policy_num
    FROM policy_data WHERE id = ?
""", (policy_id,))
    policy = cursor.fetchone()
    conn.close()

    if policy:
        policy_data = dict(zip([column[0] for column in cursor.description], policy))
        print("Policy data retrieved for edit:", policy_data)  # Debug statement
        return render_template('index.html', policy=policy_data)

    else:
        # Redirect if policy not found
        return redirect('/database')
...
@app.route('/create_new_policy')
def create_new_policy():
    # Add a unique timestamp to the URL
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
    return redirect(url_for('index', ts=timestamp))
...
def store_pdf():
    try:
        ...
        form_data = request.form  # Extract all form data at once
        policy_num = form_data.get('policy_number').strip()
        ...
        # Check if the policy number exists in the database
        cursor.execute("SELECT COUNT(*) FROM policy_data WHERE policy_num = ?", (policy_num,))
        record_count = cursor.fetchone()[0]

        # If the policy exists, prompt for confirmation to update
        if record_count > 0:
            conn.close()  # Close the connection since no new data is inserted here
            return jsonify({
                "exists": True,
                "message": "A policy with this number already exists. Do you want to update it?"
            })

        # If the policy doesn't exist, proceed to store as a new entry
        store_customer_data(form_data)
        print("store_customer_data completed")

        # Extract required form data for further actions
        vehicle_reg_number = form_data.get('vehicle_reg_number').strip()
        title = form_data.get('title').strip()
        first_name = form_data.get('First_name').strip()
        last_name = form_data['last_name'].strip()
        effective_date = form_data.get('effective_date')
        effective_time = form_data.get('effective_time')
        expiry_date = form_data.get('expiry_date')
        expiry_time = form_data.get('expiry_time')
        address = form_data['address'].strip()
        address2 = form_data['address2'].strip()
        address3 = form_data['address3'].strip()
        postcode = form_data['postcode'].strip()
        telephone = form_data['telephone'].strip()
        email = form_data['email'].strip()
        sex = form_data['sex'].strip()
        dob = form_data['dob']
        make = form_data['make'].strip()
        model = form_data['model'].strip()
        vehicle_value_from = form_data['vehicle_value_from'].strip()
        vehicle_value_to = form_data['vehicle_value_to'].strip()
        insurance_premium_tax = form_data['insurance_premium_tax'].strip()
        ...
        # Upload PDFs to GitHub repository
        ...
        # Generate and upload HTML content for the policy page
        ...
        return jsonify({"exists": False, "message": success_message})
        
    except Exception as e:
        error_message = f"An error occurred while storing PDFs and HTML: {str(e)}"
        print(error_message)  # Debugging error message
...
def update_policy():
    ...
    cursor.execute('''
        UPDATE policy_data
        SET first_name = ?, last_name = ?, dob = ?, sex = ?, title = ?, 
            address = ?, address2 = ?, address3 = ?, postcode = ?, telephone = ?, 
            email = ?, make = ?, model = ?, vehicle_reg_number = ?, 
            effective_date = ?, effective_time = ?, expiry_date = ?, 
            expiry_time = ?, vehicle_value_from = ?, vehicle_value_to = ?, duration = ?, 
            kgm_insurer_premium = ?, insurance_premium_tax = ?, tempcover_fee = ?, 
            total_charged = ?
        WHERE policy_num = ?
    ''', (
        form_data['First_name'], form_data['last_name'], form_data['dob'], form_data['sex'], 
        form_data['title'], form_data['address'], form_data['address2'], form_data['address3'], 
        form_data['postcode'], form_data['telephone'], form_data['email'],
        form_data['make'], form_data['model'], form_data['vehicle_reg_number'],
        form_data['effective_date'], form_data['effective_time'], form_data['expiry_date'], 
        form_data['expiry_time'], form_data['vehicle_value_from'], form_data['vehicle_value_to'], 
        form_data['duration'], form_data['kgm_insurer_premium'], form_data['insurance_premium_tax'], 
        form_data['tempcover_fee'], form_data['total_charged'], policy_num
    ))
    conn.commit()
    conn.close()
...
def create_policy_table():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS policy_data (
            ...
            expiry_date DATE,
            expiry_time TIME,
            vehicle_value_from REAL,
            vehicle_value_to REAL,
            duration TEXT,
            kgm_insurer_premium REAL,
            insurance_premium_tax REAL,
            tempcover_fee REAL,
            total_charged REAL,
            policy_num TEXT
        )
...
create_policy_table()  # Call the function to ensure the table is created

def store_customer_data(form_data):
    effective_date = form_data['effective_date']
    effective_time = form_data['effective_time']
    expiry_date = form_data['expiry_date']
    expiry_time = form_data['expiry_time']
    ...
    try:
        cursor.execute('''
            INSERT INTO policy_data (
                first_name, last_name, dob, sex, title, address, address2, address3, 
                postcode, telephone, email, make, model, vehicle_reg_number, 
                effective_date, effective_time, expiry_date, expiry_time, vehicle_value_from,
                vehicle_value_to, duration, kgm_insurer_premium, insurance_premium_tax, 
                tempcover_fee, total_charged, policy_num
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            form_data['First_name'], form_data['last_name'], form_data['dob'], form_data['sex'],
            ...
            form_data['vehicle_value_from'], form_data['vehicle_value_to'], duration,
            form_data['kgm_insurer_premium'], form_data['insurance_premium_tax'],
            form_data['tempcover_fee'], form_data['total_charged'], form_data['policy_number']
        ))
        conn.commit()
    except Exception as e:
